              ...
              <div className="absolute top-full left-0 mt-2 w-64 bg-brand-dark border border-white/10 rounded-xl shadow-2xl p-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                <div className="grid grid-cols-1 gap-2">
                  {SERVICES.slice(0, 6).map(s => (
                    <a key={s.slug} href={`/services/${s.slug}`} className="hover:text-brand-secondary text-xs uppercase tracking-widest py-1 block">
                      {s.name}
                    </a>
                  ))}
                  <a href="/services" className="text-brand-secondary text-xs font-black uppercase tracking-widest mt-2 block border-t border-white/5 pt-2">
                    All Services &rarr;
                  </a>
                </div>
              </div>
            </div>
            <a href="/locations" className="hover:text-brand-secondary transition-colors uppercase tracking-wider">Locations</a>
            <a href="/blog" className="hover:text-brand-secondary transition-colors uppercase tracking-wider">Blog</a>
            <a href="/about" className="hover:text-brand-secondary transition-colors uppercase tracking-wider">About</a>
            <a
        ...
        <div className="lg:hidden bg-brand-dark border-t border-white/10 py-8 px-4 space-y-6 overflow-y-auto max-h-[80vh]">
          <div className="space-y-4">
            ...
            {SERVICES.slice(0, 5).map(s => (
              <a key={s.slug} href={`/services/${s.slug}`} onClick={() => setMobileMenuOpen(false)} className="block font-bold text-white text-sm uppercase tracking-wider pl-4 border-l border-white/10 py-1">
                {s.name}
              </a>
            ))}
          </div>
          <a href="/locations" onClick={() => setMobileMenuOpen(false)} className="block font-bold text-white text-lg uppercase tracking-wider">Locations</a>
          <a href="/blog" onClick={() => setMobileMenuOpen(false)} className="block font-bold text-white text-lg uppercase tracking-wider">Blog</a>
          <a href="/about" onClick={() => setMobileMenuOpen(false)} className="block font-bold text-white text-lg uppercase tracking-wider">About</a>
